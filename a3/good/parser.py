...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.shift_id:
            if self.next >= len(self.sentence):
                raise ValueError("shift when buffer is empty")
            ...
            if len(self.stack) < 2:
                raise ValueError("right arc when stack length less than 2")
            ...
            if len(self.stack) < 2:
                raise ValueError("left arc when stack length less than 2")
    ...
    def _get_n_most_deps(self, sentence_idx, isRight = False, n = None):

        dependants = [x[1] for x in self.arcs if x[0] == sentence_idx]

        dependants = sorted(dependants, reverse = isRight)
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        '''
        ### BEGIN STUDENT CODE
        deps = self._get_n_most_deps(sentence_idx, isRight = False, n = n)
        ### END STUDENT CODE
        return deps
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        '''
        ### BEGIN STUDENT CODE
        deps = self._get_n_most_deps(sentence_idx, isRight = True, n = n)

        ### END STUDENT CODE
        return deps

    def _is_dependantOf(self, graph, a, b):

        depDic = graph.nodes[a]['deps']
        for dep in depDic:
            dependants = depDic[dep]
            if b in dependants:
                return dep
    ...
    def _is_fulfilled(self, graph, a):

        dependants = [x[1] for x in self.arcs if x[0] == a]
        n = 0
        depDic = graph.nodes[a]['deps']
        for dep in depDic:
            n += len(depDic[dep])

        return len(dependants) >= n
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        if len(self.stack) == 1:
            ...
            d =  self._is_dependantOf(graph, self.stack[-1], self.stack[-2] )

            # if d != False and self._is_fulfilled(graph, self.stack[-2]):
            ...
            if d != False:
                ...
                d = self._is_dependantOf(graph, self.stack[-2], self.stack[-1] )
                if d != False and self._is_fulfilled(graph, self.stack[-1]):
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        fs = []
        for i, (a, b) in enumerate(model.predict(batch)):
            finished = True
            try:
                batch[i].parse_step(a,b)
                if not batch[i].complete:
                    finished = False
        ...
        batchLeft = [batch[i] for i in range(len(fs)) if fs[i] == False]
        unfinished_parses = batchLeft + unfinished_parses[batch_size:]
