# Name: Yuhan Shao; LoginID: shaoyuha; StudentNumber: 1002281327
...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### BEGIN YOUR CODE
        with tf.variable_scope("id_placeholders"):
            self.word_id_placeholder = tf.placeholder(shape=[None, self.config.n_word_features],
                                                         dtype=tf.int32, name="word_id")
            self.tag_id_placeholder = tf.placeholder(shape=[None, self.config.n_tag_features],
                                                        dtype=tf.int32, name="tag_id")
            self.deprel_id_placeholder = tf.placeholder(shape=[None, self.config.n_deprel_features],
                                                        dtype=tf.int32, name="deprel_id")
        with tf.variable_scope("class_placeholder"):
            self.class_placeholder = tf.placeholder(shape=[None, self.config.n_classes],
                                                     dtype=tf.float32, name="batch_one_hot_targets")
        with tf.variable_scope("dropout_placeholder"):
    ...
    def create_feed_dict(
        ...
        if word_id_batch is not None:
            feed_dict.update({self.word_id_placeholder: word_id_batch})

        if tag_id_batch is not None:

            feed_dict.update({self.tag_id_placeholder: tag_id_batch})

        if deprel_id_batch is not None:

            feed_dict.update({self.deprel_id_placeholder: deprel_id_batch})

        if class_batch is not None:

            feed_dict.update({self.class_placeholder: class_batch})

        if dropout is not None:

             feed_dict.update({self.dropout_placeholder: dropout})
    ...
    def add_embeddings(self):
        ...
        ** Embedding matrices should be variables, not constants! **
        ...
        """
        ### BEGIN YOUR CODE
        word_embeddings = tf.reshape(tf.nn.embedding_lookup(tf.Variable(self.word_embeddings), self.word_id_placeholder),
                    [-1, self.config.n_word_features*self.config.embed_size])

        matInit = xavier_weight_init()

        tag_embeddings = tf.reshape(tf.nn.embedding_lookup(tf.Variable(matInit([self.config.n_tag_ids, self.config.embed_size])), self.tag_id_placeholder),[-1,self.config.n_tag_features*self.config.embed_size])

        deprel_embeddings = tf.reshape(tf.nn.embedding_lookup(tf.Variable(matInit([self.config.n_deprel_ids,self.config.embed_size])), self.deprel_id_placeholder),[-1,self.config.n_deprel_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        ### BEGIN YOUR CODE
        matInit = xavier_weight_init()
        W_w = tf.get_variable("W_w", [self.config.n_word_features * self.config.embed_size, self.config.hidden_size], initializer=matInit)
        W_t = tf.get_variable("W_t", [self.config.n_tag_features * self.config.embed_size, self.config.hidden_size], initializer=matInit)
        W_d = tf.get_variable("W_d", [self.config.n_deprel_features * self.config.embed_size, self.config.hidden_size], initializer=matInit)
        b1 = tf.get_variable("b1", [self.config.hidden_size], initializer=tf.zeros_initializer())
        U = tf.get_variable("U", [self.config.hidden_size, self.config.n_classes], initializer=matInit)
...
def main(debug):
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        if not debug:
            ...
            stdout.flush()
            las,uas = model.eval(test_sents, test_arcs, isTest = True)
            if las:
                print("Test LAS: ", end='')
